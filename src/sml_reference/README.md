...
Once the user's memory reconstruction is successfully completed, the application produces a deterministic 256-bit key (`md_key_v4`). This reference engine accepts that existing key and performs authenticated file encryption and decryption using **ChaCha20-Poly1305** together with the SML container format.
