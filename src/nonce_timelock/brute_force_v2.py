# ============================================================
# SecretMemoryLocker PSQC Tool (nonce_hash_V2 recovery)
#
...
# =========================
# LOAD FILE (DYNAMIC PARSING)
# =========================

def load_psq():
    for f in os.listdir("."):
        if f.lower().endswith(".psq"):
            ...
            file_prefix = content[:start_idx]
            json_str = content[start_idx:]

            try:
                data = json.loads(json_str)
                return f, data, file_prefix
...
def rewrite(file_path, data, file_prefix, nonce, hash_key, hint_key):
    new = dict(data)


    if hint_key != "nonce":
        new.pop(hint_key, None)


    if hash_key:
        new.pop(hash_key, None)


    new["nonce"] = nonce
	
    if file_prefix.startswith("TL"):
        file_prefix = file_prefix[2:]

    with open(file_path, "w", encoding="utf-8") as f:

        f.write(file_prefix + json.dumps(new))
...
def main():
    file_path, data, file_prefix = load_psq()

    if not data:
        print("[!] No PSQ file with valid JSON found")
        return
    ...
    for key, val in data.items():
        if isinstance(val, str):
            ...
            if key.startswith("nonce") and "*" in val:
                nonce_hint = val
                hint_key = key
            

            elif key.startswith("nonce_hash"):
                target_hash = val
                hash_key = key

    if not nonce_hint or not target_hash:
        print("[!] Invalid structure: Missing 'nonce' hint or 'nonce_hash'")
        return

    print("[i] PSQC Brute-force Engine Started")
    print(f"[i] File: {file_path}")
    print(f"[i] Detected Prefix: {file_prefix}")
    print(f"[i] Detected Hint Key: '{hint_key}'")
    print(f"[i] Detected Hash Key: '{hash_key}'")
    ...
    if nonce:
        rewrite(file_path, data, file_prefix, nonce, hash_key, hint_key)
